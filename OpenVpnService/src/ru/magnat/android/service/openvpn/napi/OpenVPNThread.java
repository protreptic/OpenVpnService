...
import ru.magnat.android.service.openvpn.napi.VpnStatus.LogItem;
...
public class OpenVPNThread implements Runnable {
	...
	public OpenVPNThread(OpenVpnService service, String[] argv,
			Map<String, String> processEnv, String nativelibdir) {
		mArgv = argv;
	...
	public void run() {
		try {
			...
			try {
				...
				VpnStatus.logError("Illegal Thread state: "
						+ ite.getLocalizedMessage());
			} catch (InterruptedException ie) {
				VpnStatus.logError("InterruptedException: "
						+ ie.getLocalizedMessage());
			}
			if (exitvalue != 0)
				VpnStatus.logError("Process exited with exit value "
						+ exitvalue);
			...
			if (mDumpPath != null) {
				try {
					...
					for (LogItem li : VpnStatus.getlogbuffer()) {
						String time = timeformat.format(new Date(li
								.getLogtime()));
					...
					VpnStatus.logError("Writing minidump log: "
	...
	private void startOpenVPNThreadArgs(String[] argv, Map<String, String> env) {
		...
		try {
			...
			while (true) {
				...
				// 1380308330.240114 18000002 Send to HTTP proxy:
				// 'X-Online-Host: bla.blabla.com'
			...
			VpnStatus.logException(
					"Error reading from output of OpenVPN process", e);
			stopProcess();
		}
	...
	private String genLibraryPath(String[] argv, ProcessBuilder pb) {
		...
		String applibpath = argv[0].replace("/cache/" + VpnProfile.MINIVPN,
				"/lib");
